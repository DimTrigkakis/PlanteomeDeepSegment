...
def segmentation_processing():
	...
	my_parameters = {'bins': 8, 'sigma': 7.0, 'interaction_cost': 50, 'stroke_dt': True, 'hard_seeds': True, 'stroke_var': 1}
...
if network == "Simple classification":
	...
	dualnet = models.resnet34(pretrained=False)
	dualnet.fc = nn.Linear(512 * 1, 5)
	criterion = nn.NLLLoss()
	m = nn.LogSoftmax()
	dualnet.load_state_dict(torch.load("/home/dimitris/Bisque/modules/PlanteomeDeepSegment/DeepModels/resnet34model.pth"))
	dualnet.eval()
	#data = Variable(img).cuda()
	#output_t = dualnet(data)
	#pred_t = output_t.data.cpu().max(1)[1]


	#model = VGG(num_classes=5)
	#model2 = ResNet()
	#model.load_state_dict(torch.load('/home/dimitris/Bisque/modules/SkeletonPython/DeepModels/vgg16_single.pth',map_location=lambda storage, loc: storage))
	#model.eval()

	transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.4379,0.4644,0.3316),(0.318,0.308,0.337))])
